...
async def test_generate_embedding():
    ...
    assert len(embedding) == 1024  # Dimension par défaut = 1024 (MockEmbeddingService)
    assert all(
...
async def test_dimension_property():
    ...
    assert service.dimension == 1024  # Dimension par défaut = 1024 (MockEmbeddingService)
...
async def test_batch_embedding():
    ...
    assert all(
        len(emb) == 1024 for emb in embeddings
    )  # Dimension par défaut = 1024 (MockEmbeddingService)
