...
def mock_memory_repo():
    ...
    mock_repo.list_memories.return_value = (
        [],
        0,
    )  # Par défaut, aucun résultat et count=0
...
def test_get_memory_with_exception(client_with_mock_repo):
    """Teste la gestion d'une exception lors de la récupération d'une mémoire."""
    client, app = client_with_mock_repo
    mock_repo = app.dependency_overrides[get_memory_repository]()

    # 1. Préparation
    test_memory_id = uuid.uuid4()
    mock_repo.get_by_id.side_effect = Exception("Test database error")
...
def test_create_memory_error(client_with_mock_repo):
    """Teste la gestion d'une erreur lors de la création d'une mémoire."""
    client, app = client_with_mock_repo
    mock_repo = app.dependency_overrides[get_memory_repository]()
    ...
    # Configurer le mock pour qu'il lève une exception
    mock_repo.add.side_effect = Exception("Erreur de création en base de données")
...
def test_update_memory_not_found(client_with_mock_repo):
    ...
    # 1. Préparation
    test_memory_id = uuid.uuid4()
    update_data = {"content": {"message": "Updated content"}}
...
def test_update_memory_error(client_with_mock_repo):
    """Teste la gestion d'une erreur lors de la mise à jour d'une mémoire."""
    client, app = client_with_mock_repo
    mock_repo = app.dependency_overrides[get_memory_repository]()
    ...
    # Configurer le mock pour qu'il lève une exception
    mock_repo.update.side_effect = Exception("Erreur de mise à jour en base de données")
...
def test_delete_memory_error(client_with_mock_repo):
    """Teste la gestion d'une erreur lors de la suppression d'une mémoire."""
    client, app = client_with_mock_repo
    mock_repo = app.dependency_overrides[get_memory_repository]()

    # 1. Préparation
    test_memory_id = uuid.uuid4()
    mock_repo.delete.side_effect = Exception("Erreur de suppression en base de données")
...
def test_list_memories_with_filters(client_with_mock_repo):
    ...
    response = client.get(
        "/v0/memories/?memory_type=episodic&event_type=user_input&role_id=1&session_id=test-session"
    )
...
def test_list_memories_error(client_with_mock_repo):
    """Teste la gestion d'une erreur lors de la récupération de la liste des mémoires."""
    client, app = client_with_mock_repo
    mock_repo = app.dependency_overrides[get_memory_repository]()

    # 1. Préparation - Simuler une erreur
    mock_repo.list_memories.side_effect = Exception(
        "Erreur lors de la récupération des mémoires"
    )
...
def test_invalid_path_for_memories():
    """Teste que l'ancien chemin /v1/memories n'est plus utilisable."""
    client = TestClient(app)

    # Test que le nouveau chemin fonctionne
    response_new = client.get("/v0/memories/")
    assert (
        response_new.status_code != 404
    )  # Peut être 200 ou autre statut selon l'implémentation

    # Test que l'ancien chemin ne fonctionne pas
    response_old = client.get("/v1/memories/")
