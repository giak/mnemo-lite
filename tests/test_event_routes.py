...
def test_update_event_metadata_success(client_with_mock_repo):
    ...
    response = client.patch(
        f"/v1/events/{test_event_id}/metadata", json=metadata_update
    )
...
def test_filter_events_by_metadata(client_with_mock_repo):
    ...
    # 3. Vérification
    mock_repo.filter_by_metadata.assert_called_once_with(
...
def test_search_events_by_embedding(client_with_mock_repo):
    ...
    response = client.post(
        f"/v1/events/search/embedding?limit={test_limit}", json=test_embedding
    )
...
@pytest.mark.skip(
    reason="Problème persistant de mocking/patch avec TestClient sur cette route"
)
def test_get_event_success_with_patch(client: TestClient):
    ...
    with patch(
        "db.repositories.event_repository.EventRepository.get_by_id",
...
def test_get_event_not_found_with_patch(client: TestClient):
    ...
    with patch(
        ...
        assert response.json() == {
            "detail": "Événement avec ID " + str(test_event_id) + " non trouvé"
        }
