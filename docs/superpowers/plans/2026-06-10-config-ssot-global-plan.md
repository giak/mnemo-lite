...
| Auto-Import | `CLAUDE_PROJECTS_DIR`, `CODEBUFF_DIR`, `OPENCODE_DIR`, `ACTIVE_PROJECT`, `ENABLE_AUTO_IMPORT`, `CONVERSATION_WATCHER_ENABLED`, `POLL_INTERVAL`, `IMPORT_HISTORICAL`, `TYPESCRIPT_LSP_ENABLED` |
| MCP | `MCP_PRIVACY_ENABLED`, `MCP_TRANSPORT`, `MCP_HTTP_HOST`, `MCP_HTTP_PORT`, `MCP_AUTH_MODE` |
| Features | `ENTITY_EXTRACTION_ENABLED`, `QUERY_UNDERSTANDING_ENABLED`, `QUERY_UNDERSTANDING_FALLBACK`, `USE_ONNX` |
