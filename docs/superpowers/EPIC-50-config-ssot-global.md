...
Un `AppSettings` etendu (38 champs au total) couvrant TOUTE la configuration de l'application. Une simple modification du `.env` propage automatiquement la valeur partout. Les scripts importent `get_settings()` comme l'API.

### Domaines de configuration

| Domaine | Champs | Nb |
|---------|--------|----|
| Embedding (existant) | EMBEDDING_MODEL, DIMENSION, BACKEND, MODE, CODE_*, DEVICE, CACHE_SIZE, AUTO_GENERATE, FAIL_STRATEGY, SOURCE_FIELDS, GLINER_* | 17 |
...
| Auto-Import | CLAUDE_PROJECTS_DIR, CODEBUFF_DIR, OPENCODE_DIR, ACTIVE_PROJECT, ENABLE_AUTO_IMPORT, CONVERSATION_WATCHER_ENABLED, POLL_INTERVAL, IMPORT_HISTORICAL, TYPESCRIPT_LSP_ENABLED | 9 |
| MCP Server | MCP_PRIVACY_ENABLED, MCP_TRANSPORT, MCP_HTTP_HOST, MCP_HTTP_PORT, MCP_AUTH_MODE | 5 |
| Feature Flags | ENTITY_EXTRACTION_ENABLED, QUERY_UNDERSTANDING_ENABLED, QUERY_UNDERSTANDING_FALLBACK, USE_ONNX | 4 |
...
- **Story 50.1 :** Etendre AppSettings avec les 26 nouveaux champs (5 pts)
