# MnemoLite - UI Architecture (Vue 3 + Alpine.js Sprinkles)
...
Ce document détaille l'architecture de l'interface utilisateur (UI) choisie pour MnemoLite, basée sur une approche hybride combinant **Vue 3** pour les interactions serveur et **Alpine.js** (ou potentiellement Hyperscript) pour les améliorations client légères ("sprinkles").
...
L'approche "Vue 3 + Alpine.js Sprinkles" a été préférée aux alternatives (Pure Vue 3, SPA dédiée) pour les raisons suivantes :

*   **Équilibre optimal :** Conserve la simplicité et l'intégration backend de l'approche "Pure Vue 3" tout en palliant ses limitations UX grâce à Alpine.js pour les micro-interactions côté client.
...
*   **Performance :** Privilégie le rendu serveur initial rapide et les échanges légers (HTML partiel via Vue 3, JS minimaliste via Alpine).
...
*   **Vue 3 (Interactions Serveur) :** Bibliothèque JavaScript légère qui permet de déclencher des requêtes AJAX vers le serveur via des attributs HTML (`hx-get`, `hx-post`, `hx-trigger`, etc.) et de remplacer des parties du DOM avec le HTML retourné par le serveur. Gère le *gros* des interactions dynamiques liées aux données.
...
sequenceDiagram
    ...
    FastAPI-->>-Browser: Réponse HTML (inclut Vue 3 + Alpine.js)

    Note over Browser: Interaction Utilisateur (Vue 3)
    Browser->>Browser: Clic sur <button hx-get="/ui/update-list">
    Browser->>+FastAPI: GET /ui/update-list (Requête Vue 3)
    ...
    Browser->>Browser: Vue 3 remplace #target-div avec le fragment reçu
...
## 6. Modèles d'Interaction (Vue 3 + Alpine.js)
...
| **Soumission de formulaire** | Vue 3 (`hx-post`) | Envoie les données au serveur, remplace une partie du DOM (ou la page entière) avec la réponse serveur.        | Validation *côté client* avant envoi, gestion de l'état "loading". |
| **Filtrage / Tri de listes** | Vue 3 (`hx-get`)  | Envoie les paramètres de filtre/tri au serveur, remplace la liste affichée avec le fragment HTML mis à jour. | Gestion de l'état actif des boutons de filtre/tri (UI).       |
| **Pagination**               | Vue 3 (`hx-get`)  | Récupère et affiche la page suivante/précédente de données sous forme de fragment HTML.                      | Mise à jour de l'indicateur de page active.                 |
...
| **Mises à jour temps réel**  | Vue 3 (SSE/WebSockets Ext.) | Si nécessaire plus tard, utiliser les extensions Vue 3 pour écouter les événements serveur.           | Peut déclencher des logiques Alpine à la réception d'événements. |
...
*   **Utiliser Alpine.js pour :**
    ...
    *   Fournir un **feedback visuel immédiat** avant ou après une action Vue 3 (ex: état "loading", highlight d'un élément ajouté).
...
*   **Éviter avec Alpine.js :**
    *   Récupérer ou envoyer des données au serveur (c'est le rôle de Vue 3).
...
*   **Organisation du Code :** Maintenir la clarté entre les attributs Vue 3 et Alpine.js sur des éléments complexes.

## 9. Conclusion

L'architecture "Vue 3 + Alpine.js Sprinkles" représente un compromis pragmatique et puissant pour MnemoLite. Elle offre une base solide, performante et maintenable, alignée sur les contraintes et objectifs du projet, tout en permettant une expérience utilisateur agréable et réactive pour les cas d'usage principaux définis. 