...
<template>
  <div class="container mx-auto px-4 py-6">
    ...
    <Transition name="slide">
      <div
        v-if="showHelp"
        class="scada-panel mb-6 bg-slate-800/30"
      >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm font-mono">
          <!-- Chart help -->
          <div>
            <div class="text-cyan-400 uppercase text-xs mb-2">
              Graphique Latence
            </div>
            <div class="space-y-1 text-slate-400">
              ...
              <div class="text-slate-500 mt-2">
                Idéal: &lt;200ms avg. Attention: &gt;500ms. Critique: &gt;2s
              </div>
            </div>
          </div>
          <!-- Severity help -->
          <div>
            ...
            <div class="space-y-1 text-slate-400">
              <div><span class="scada-led scada-led-red" /> Critical — Action immédiate requise</div>
              <div><span class="scada-led scada-led-yellow" /> Warning — À surveiller</div>
              <div><span class="scada-led scada-led-green" /> Info — Tout va bien</div>
            </div>
          </div>
          <!-- Actions help -->
          <div>
    ...
    <div class="scada-panel mb-6">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div class="flex items-center gap-4 ml-auto">
          <div
            ...
            title="Temps de réponse moyen sur 24h"
          >
            <span class="w-3 h-0.5 bg-cyan-400 inline-block" />
            <span class="text-xs font-mono text-slate-400">Avg: <span class="text-cyan-400">{{ chartAvg }}ms</span></span>
          </div>
          <div
            class="flex items-center gap-2"
            title="Requête la plus lente sur 24h"
          >
            <span class="w-3 h-0.5 bg-red-400 inline-block" />
            <span class="text-xs font-mono text-slate-400">Max: <span class="text-red-400">{{ chartMax }}ms</span></span>
          </div>
          <div
            class="flex items-center gap-2"
            title="Nombre d'intervalles horaires avec des données"
          >
      ...
      <div class="flex items-center gap-6 mt-3 text-xs font-mono text-slate-500">
        ...
        <div class="flex items-center gap-2">
          <span class="w-3 h-0.5 bg-amber-400 inline-block border-dashed" />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="scada-panel">
        <div class="flex items-center gap-3 mb-4">
          ...
          <span
            class="text-[10px] font-mono text-slate-600 ml-2"
            title="Nombre d'alertes non acquittées par type"
          >
            (non-acknowledged)
          </span>
        </div>
        ...
        <div class="space-y-3">
          ...
          >
            <div class="flex items-center justify-between mb-1">
              <div class="flex items-center gap-3">
                <span
                  class="scada-led"
                  :class="`scada-led-${severityLed[info.severity] || 'yellow'}`"
                />
                <span class="font-mono text-sm text-slate-300 uppercase">{{ formatAlertType(type) }}</span>
              </div>
              <div class="flex items-center gap-3">
                <span
                  class="scada-data text-sm font-mono"
                  :class="info.unacked > 0 ? 'text-red-400' : 'text-slate-500'"
                >
          ...
          <div
            ...
            class="scada-data text-slate-500 text-center py-8"
          >
            NO ALERTS
          </div>
        </div>
        ...
        <div class="space-y-2 max-h-[50vh] overflow-y-auto pr-1">
          ...
          >
            <div class="flex items-center gap-2 mb-1">
              <span
                class="scada-led"
                :class="`scada-led-${severityLed[alert.severity] || 'yellow'}`"
              />
