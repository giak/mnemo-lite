...
async def create_memory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la création de la mémoire: {str(e)}",
        )
...
async def get_memory(
    ...
    try:
        ...
        if memory is None:
            raise HTTPException(
                status_code=404, detail=f"Mémoire avec ID {memory_id} non trouvée"
            )
        return memory
        ...
        logger.error(
            f"Erreur lors de la récupération de la mémoire {memory_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
...
async def update_memory(
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la mise à jour de la mémoire {memory_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la mise à jour de la mémoire: {str(e)}",
        )
...
async def delete_memory(
    ...
    try:
        ...
        logger.error(
            f"Erreur lors de la suppression de la mémoire {memory_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la suppression de la mémoire: {str(e)}",
        )
...
async def list_memories(
    repo: Annotated[MemoryRepositoryProtocol, Depends(get_memory_repository)],
    limit: int = Query(
        10, ge=1, le=100, description="Nombre maximum de résultats à retourner"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la récupération des mémoires: {str(e)}",
        )
