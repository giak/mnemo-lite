...
class SearchResponse(BaseModel):
    ...
    data: List[Union[Memory, EventModel]]
    meta: SearchResponseMeta

# Nouvelle route principale qui supporte à la fois la recherche par métadonnées et la recherche vectorielle
@router.get("/", response_model=SearchResponse)
async def search(
    filter_metadata: Optional[str] = Query(None, description="Filtre JSON sur les métadonnées"),
    vector_query: Optional[str] = Query(None, description="Requête textuelle pour recherche vectorielle"),
    limit: int = Query(10, ge=1, le=100, description="Nombre maximum de résultats"),
    offset: int = Query(0, ge=0, description="Décalage pour la pagination"),
    ts_start: Optional[str] = Query(None, description="Timestamp de début (format ISO)"),
    ...
    if vector_query:
        try:
            ...
            logger.error(f"Erreur lors du traitement de la requête vectorielle: {str(e)}")
            raise HTTPException(
                status_code=422,
    ...
    results, total_hits = await memory_search_service.search_hybrid(
        query=vector_query if vector_query and not vector_query.startswith('[') else None,
        embedding=embedding,
