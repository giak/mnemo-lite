...
class EventProcessor(EventProcessorProtocol):
    ...
    async def process_event(self, event: EventModel) -> Dict[str, Any]:
        ...
        try:
            ...
            if not event.embedding:
                ...
                try:
                    ...
                    updated_event = await self.event_repository.update_metadata(
                        event_id=event.id, metadata={**metadata, "has_embedding": True}
                    )
                    if updated_event:
                        logger.info(
                            f"Métadonnées de l'événement {event.id} mises à jour"
                        )
                    else:
                        logger.warning(
                            f"Impossible de mettre à jour les métadonnées de l'événement {event.id}"
                        )
            ...
            logger.error(
                f"Erreur lors du traitement de l'événement {event.id}: {str(e)}"
            )
    ...
    async def generate_memory_from_event(self, event: EventModel) -> Optional[Memory]:
        ...
        try:
            ...
            memory = Memory(
                ...
                metadata={
                    **enriched_metadata,
                    ...
                    "memory_created_at": datetime.now(UTC).isoformat(),
                },
            ...
            logger.error(
                f"Erreur lors de la génération de mémoire à partir de l'événement {event.id}: {str(e)}"
            )
            return None
