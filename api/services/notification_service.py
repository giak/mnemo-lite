...
class NotificationService(NotificationServiceProtocol):
    ...
    async def send_notification(
        ...
        try:
            ...
            # Utiliser des métadonnées par défaut si non fournies
            meta = metadata.copy() if metadata else {}

            # Ajouter des informations à la notification
            meta["status"] = "logged_only" if not self.smtp_configured else "sent"
            ...
            logger.error(
                f"Erreur lors de l'envoi de notification à {user_id}: {str(e)}"
            )
            return False

    async def broadcast_notification(
        self, message: str, user_ids: Optional[List[str]] = None
    ) -> Dict[str, bool]:
        """
        ...
        try:
            if not user_ids:
                logger.warning(
                    "Aucun destinataire spécifié pour la diffusion de notification"
                )
                return {}

            logger.info(f"Diffusion de notification à {len(user_ids)} utilisateurs")

            # Générer un ID de diffusion unique
            broadcast_metadata = {
                "type": "broadcast",
                "broadcast_size": len(user_ids),
                "broadcast_id": str(datetime.now(UTC).timestamp()),
            }
            ...
            for user_id in user_ids:
                success = await self.send_notification(
                    user_id=user_id, message=message, metadata=broadcast_metadata
                )
            ...
            logger.info(
                f"Diffusion terminée: {success_count}/{len(user_ids)} notifications envoyées"
            )
