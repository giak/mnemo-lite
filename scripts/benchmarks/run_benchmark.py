...
try:
    ...
    print("Assurez-vous que le script est exécuté dans un environnement où 'db' et 'db.repositories' sont accessibles depuis PYTHONPATH=/app (ex: conteneur api)")
    sys.exit(1)
...
async def run_query_scenario(
    ...
    for i in range(num_runs):
        ...
        try:
            ...
            latencies_ms.append((end_time - start_time) / 1_000_000) # Convertir ns en ms
            print(f"  Run {i+1}/{num_runs}: {latencies_ms[-1]:.2f} ms, {len(results)} résultats", end='\r')
        except Exception as e:
...
async def main(db_dsn: str, num_runs: int, embedding_dim: int, top_k: int, limit: int):
    ...
    db = Database(db_dsn)
    db_pool = await db.get_pool() # Utilise la méthode de la classe Database pour obtenir le pool
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Lance un benchmark léger sur l'API de recherche MnemoLite.")
