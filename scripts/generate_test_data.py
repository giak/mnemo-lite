...
DATABASE_URL = os.getenv(
    "TEST_DATABASE_URL",
    "postgresql://mnemo:mnemopass@db:5432/mnemolite_test"
).replace("+asyncpg", "")  # Conversion pour asyncpg natif
...
# Catégories de test
CATEGORIES = [
    "note", "email", "message", "document", "image", "code", 
    "meeting", "task", "idea", "reference"
]

# Sources de test
SOURCES = [
    "web", "api", "manual", "system", "import", "query", 
    "search", "feed", "notification"
]

# Tags de test
TAGS = [
    "important", "urgent", "follow-up", "archived", "draft", 
    "personal", "work", "project", "reference", "learning",
    "high-priority", "low-priority", "team", "private"
]
...
async def generate_event_metadata(category, source):
    ...
    if category == "task":
        metadata["status"] = random.choice(["pending", "in-progress", "completed", "blocked"])
...
async def insert_events(conn, num_events):
    ...
    for i in range(num_events):
        ...
        timestamp = datetime.datetime.now(timezone.utc) - timedelta(days=days_ago, hours=hours_ago)
        ...
        embedding_list = await generate_random_embedding() if random.random() < 0.8 else None
        ...
        await conn.execute('''
            ...
            VALUES ($1, $2, $3, $4, $5)
        ''', 
        uuid.uuid4(), timestamp, json.dumps(content), json.dumps(metadata), 
        embedding_formatted)
...
async def create_test_nodes_and_edges(conn):
    ...
    for i, row in enumerate(event_ids):
        ...
        await conn.execute('''
            ...
            VALUES ($1, $2, $3, $4)
        ''', 
        event_id, node_type, label, json.dumps(properties))
    ...
    for _ in range(30):
        ...
        relation_type = random.choice(["references", "follows", "precedes", "mentions", "related_to"])
        ...
        await conn.execute('''
            ...
            VALUES ($1, $2, $3, $4)
        ''', 
        source_node, target_node, relation_type, json.dumps(properties))
...
async def main():
    ...
    try:
        ...
        if count > 0:
            ...
            proceed = input("Voulez-vous ajouter plus d'événements de test? (o/n): ").lower()
            if proceed != 'o':
