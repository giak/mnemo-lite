...
def process_file(file_path: Path, dry_run: bool = False, verbose: bool = False) -> Tuple[int, Dict[str, int]]:
    """
    ...
    try:
        ...
        if not dry_run and content != original_content:
            ...
            logger.info(f"[DRY RUN] {file_path}: {total_changes} changements seraient appliqués.")
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Montre les modifications sans les appliquer")
    parser.add_argument("--verbose", action="store_true", help="Affiche des informations détaillées")
    args = parser.parse_args()
    
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # Dossier racine du projet
    ...
    if args.dry_run:
        ...
        logger.info("Pour appliquer les modifications, exécutez sans l'option --dry-run.")
